...
export function VideoCard({ video, index, series, onPlay }: VideoCardProps) {
  return (
    ...
    >
      {/* Thumbnail */}
      <div className="relative aspect-video bg-gradient-to-br from-[#0D0B1A] to-[#3D2080] overflow-hidden">
        ...
        {/* Series tag */}
        {series && (
          <span className="absolute top-2 left-2 bg-white/90 text-[#28166F] text-[10px] px-2 py-0.5 rounded-md font-semibold">
            {series.name}
          </span>
        )}
        ...
        {!series && (
          <span className="absolute top-2 left-2 bg-white/90 text-[#28166F] text-[10px] px-2 py-0.5 rounded-md font-medium">
            {video.category}
          </span>
        )}
      ...
      {/* Info */}
      <div className="p-4">
        <h3 className="text-sm font-bold text-[#28166F] leading-tight line-clamp-2 mb-1.5 group-hover:text-[#0D0B1A] transition-colors">
          {video.title}
        </h3>
  ...
  return (
    <motion.div
      ...
      className={`cursor-pointer rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border-2 ${
        isActive ? "border-[#28166F] shadow-md" : "border-transparent"
      }`}
    >
